...
- 硬件: CPU 单核 (Ryzen), 全部实验从零跑完约 2 分钟.
...
> ℹ️ **关于实验编号**. 代码里的 `experiments/04_plots.py` 与 `experiments/11_verification_plots.py` 是**出图脚本**——读 CSV 画图, 没有独立实验意义. 所以下文按**实验编号** (02 / 03 / 05 / 07 / 08 / 09 / 10 / 12 / 06 / 06b) 组织章节, exp04 的 fig1~fig3 会跟着 exp02/03 一起出现, exp11 的 fig5~fig8 会跟着 exp07 到 10 一起出现.
...
*数据来源: [`exp05_benchmark_table.csv`](exp05_benchmark_table.csv). 文献数字来自 DCDAN [1], AMDA [3], CDHM [2] 三篇论文中的横向对比表, 均为 CWRU 10 类 0HP↔3HP 任务的公开报告.*
...
到这里 10 个实验章节全部报完. 回到项目缘起那一节抛出的问题:
...
4. **未测真实边缘硬件**. exp12 的延迟数据基于 x86 CPU 单核, 到 ARM Cortex-A 上会有 2 到 4 倍膨胀, 但数量级不变. 内存峰值、并发吞吐这些指标也未量化.
5. **LLM 层的定位是接口示例**. 本仓库不提供开箱即用的生产级维修建议 — 生成质量强依赖所选模型, 建议至少接 7B+ 本地模型或云端 API.
...
- 补一份 PU 或 MFPT 数据集的迁移实验, 让"外部有效性"从"单数据集"升级到"多数据集"
- 把 CFD 特征提取的 numpy 实现用 C 重写, 尝试把 batch=1 延迟压到 <0.1 ms
- 把 LLM 层加一层输出校验 (基于故障类型的白名单术语过滤)

**中期 (需要新数据 / 新硬件)**:

- 接入真实 ARM Cortex-A 边缘设备, 实测端到端延迟与吞吐
- 引入变转速、变载荷混合场景, 而不是当前 CWRU 的固定 4 档负载
- 探索 Cohen's d 之外的其他分布距离 (Wasserstein / KL / MMD) 对 Bot-40 挑选结果的影响

**长期 (研究方向)**:

- 我个人最想做的事: 把这套方法应用到**更多种旋转机械**的实测数据上, 而不是只在两种设备之间迁移. 但这已经不是本仓库的范畴, 是我下一阶段研究的问题.

---

## 参考

[1] DCDAN — Domain-Class Dual Adversarial Network for Cross-Domain Fault Diagnosis
[2] CDHM — Causal Disentangled Hybrid Method for Bearing Fault Diagnosis
[3] AMDA — Adversarial Multi-Domain Adaptation for Fault Diagnosis

数据集: Case Western Reserve University Bearing Data Center, <https://engineering.case.edu/bearingdatacenter>
