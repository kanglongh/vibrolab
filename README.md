...
> **跨工况轴承故障诊断的全链路复现 + ESP32-S3 上位机模式实物部署.**
> CWRU 10 类 · 0+3HP 双工况训练 → held-out 1/2HP 100% · 2 KB 模型烧进 40 元级开发板, 单窗推理 0.6 ms · 主动公开阴性结果 (exp09 未见损伤尺寸外推崩塌) 与采集侧未验证边界

![板子实时诊断](firmware_v1_5/assets/photo_board_running.jpg)

*ESP32-S3 板子实时诊断: OLED 顶端波形 · 中间中文标签 (外圈007) · 底部置信度条 (1.00) + 延迟数值 (0ms). 完整故事 + 4 段传感器降级对照视频见 [`firmware_v1_5/README.md`](firmware_v1_5/README.md).*

> CWRU 上 99% 任何方法都能刷到, 单看精度数字说明不了什么. 这个仓库讲的是**把一个课题从数据加载 → 特征工程 → 跨域验证 → 效率对照 → 传感器降级分析 → 训练导出 → 烧进 MCU 全链路做完, 并且每一步的边界都摆出来**——包括哪些是真测出来的、哪些是软件模拟的、哪些是数据集本身的天花板. 详见下文与 [已知局限](#已知局限).
...
**这个仓库就是这次迁移验证的完整记录**——数据、代码、CSV、图表、部署规格、LLM 接口示例全部公开可复现. CWRU 上刷到 99% 本身说明不了什么, 但**把整套流程、每一个数字、每一个阴性结果 (见 [exp09](outputs/VERIFICATION.md)) 和每一处未闭环的边界 (采集前端、传感器真机、跨机型)** 都放到 GitHub 上, 至少证明**我这套方法不是柱塞泵专属的偶然**——同时也让读者能自己判断哪些结论稳、哪些结论悬.

> **跨数据集有效性另开一个仓库补足**: [diagnosis-agent](https://github.com/kanglongh/diagnosis-agent) 把同一套 Cohen's d + Bot-40 直接搬到 Paderborn (PU) 轴承数据集, N09→N15 跨域精度 0.8319 (vs 全 120 维基线 0.7518). CWRU 和 PU 是不同机构、不同试验台、不同传感器、不同采样率——这套方法在 CWRU 之外仍然有效.
...
- **本仓库为单数据集 (CWRU) 验证**. 未在 MFPT / XJTU-SY 等其他公开数据集上评估. 跨数据集有效性由姊妹仓库 [diagnosis-agent](https://github.com/kanglongh/diagnosis-agent) 在 **Paderborn (PU) 轴承数据集**上补足: 同一套 Cohen's d + Bot-40 方法直接迁移到 PU, N09→N15 跨域精度 **0.8319** (vs 全 120 维 SVM 基线 0.7518). 证明这套物理特征选择范式不是 CWRU 专属.
- **上位机模式部署, 采集前端未闭环**. ESP32-S3 板子上跑的是**"特征提取 + 推理 + OLED 显示"**这条链, 信号由 PC 从 CWRU .mat 流式喂入 (相当于数据采集卡的角色). **未实现的部分**: 真实传感器 (IEPE 或 MEMS) → 运放前端 → ESP32 ADC 采样这条硬件链路——受实验室条件所限, **缺一支 IEPE 传感器 + 电荷放大器作为"金标准参照"** (没有金标准就分不清一支廉价传感器精度掉了 X% 里, 是传感器本身的锅、还是试验台/算法的锅). 因此 exp17 传感器降级是**软件模拟**, 不是真机测量.
- **传感器成本论断的真机验证已主动放弃**. 即便实验室条件允许, 工业现场对可靠性 / 防爆认证 / EMI / 供应商责任 / 可维护生态的要求, 决定了 ESP32 + 廉价 MEMS 方案不会被现役检测系统采纳——**逻辑链没有下游出口, 做完也无法转化为决策价值**. 因此本项目的价值定位是"算法侧 + 边缘部署可行性"演示 (技术资源储备), 不是"工业成本优化方案". 这是一个主动的工程判断, 不是能力缺失.
